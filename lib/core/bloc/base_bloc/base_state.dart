...
extension BaseStateExt<T> on BaseState<T> {
  bool get isSuccess =>
      maybeWhen(success: (changed, _) => true, orElse: () => false);
  bool get isNotSuccess => !isSuccess;

  bool get isLoading => maybeWhen(loading: () => true, orElse: () => false);

  bool get isFailure =>
      maybeWhen(failure: (_, __) => true, orElse: () => false);

  bool get isInit => maybeWhen(init: () => true, orElse: () => false);

  bool get hasData =>
      maybeWhen(success: (changed, data) => data != null, orElse: () => false);

  bool get hasNoData => !hasData;

  T? get data =>
      maybeWhen(success: (changed, data) => data, orElse: () => null);

  BaseError? get error =>
      maybeWhen(failure: (error, _) => error, orElse: () => null);
}
