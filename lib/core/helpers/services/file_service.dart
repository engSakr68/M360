...
class AppFileService {
  ...
  Future<List<File>?> pickFile(
    ...
    bool canAccess = await GetIt.I<PermissionServices>()
    ...
    if (canAccess) {
      ...
      FilePickerResult? pickedFiles = await FilePicker.platform.pickFiles(
        type: fileType ??
            (allowedExtensions.isNullOrEmpty ? FileType.any : FileType.custom),
      ...
      if (pickedFiles != null) {
        ...
        _showAllowedSizeExceededWarning(
            fileHasHugeSize: fileHasHugeSize, maxSize: maxSize);
          ...
          return CupertinoActionSheet(
            actions: [
              ...
              CupertinoActionSheetAction(
                onPressed: () async {
                  final images =
                      await _pickImageFiles(context, allowMultiple: allowMulti);
                  if (images != null) {
                    files = images;
                  }
                  Navigator.pop(context);
  ...
  void _showAllowedSizeExceededWarning({
    ...
    if (fileHasHugeSize.isNotEmpty) {
      ...
      AppSnackBar.showSimpleToast(
          msg: Translate.s
              .size_more_than_mg_error((maxSize) / pow(1024, 2), filesName));
    }
  }
}
