...
class XmlArgsParser {
  ...
  Map<String, dynamic> getArgsMap() {
    ...
    final argsNamesWithValues =
        rowArgs.replaceAll('&quot;', '').split('--dart-define=');

    final Map<String, dynamic> argsAsMap =
        _convertArgsToMap(argsNamesWithValues);
    return argsAsMap;
  }
  ...
  Map<String, dynamic> _convertArgsToMap(List<String> argsNamesWithValues) {
    ...
    for (var element in argsNamesWithValues) {
      ...
      if (parts.first.trim().isNotBlank && parts.last.trim().isNotBlank) {
        argsAsMap.putIfAbsent(
            parts.first.trim(), () => parts.last.trim().replaceAll('"', ''));
      }
    }
    return argsAsMap;
  }
