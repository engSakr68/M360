...
Future<void> handleConfig(AppEnvType env) async {
  final reader = DartEnvironmentConfigReader(
    envType: env,
    argsMap:
        json.decode(File('.run/json/${env.value}.json').readAsStringSync()),
  );

  /// Handle name
  print('Handle Config>> Handle name..');
  // await _setAppId(reader.getAppId());
  final nameHandler = AppNameHandler(
      enName: reader.getAppNameEN(), arName: reader.getAppNameAr());
...
class AppNameHandler {
  ...
  Future<void> _handleIOS() async {
    await reWriteFile(
        path: 'ios/en.lproj/InfoPlist.strings',
        content: _infoPlistFileContent(enName));
    await reWriteFile(
        path: 'ios/ar.lproj/InfoPlist.strings',
        content: _infoPlistFileContent(arName));
  }
